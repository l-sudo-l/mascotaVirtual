...
public class Tienda extends Lugar{
    ...
    public static void mostrarTienda(Personaje personaje){
        ...
        for (int i = 0 ; i < catalogo.length ; i++){
            ...
            System.out.println((i+1) + ") " + item.getNombre() + item.getPrecioCompra() + item.getHambre() + item.getEnergia() + item.getFelicidad() + item.getSuciedad());
        }
        ...
        if (personaje.getDinero()>=elegido.getPrecioCompra()){
            ...
            personaje.inventario().agregarCosasInventario(elegido);
        }
